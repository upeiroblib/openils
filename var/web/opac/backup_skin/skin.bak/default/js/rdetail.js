...
function _rdetailDraw(r) {
	...
	for( var i = 0; links && links.length > 0 && i < links.length; i = i + 2 ) {
		...
		if( href.match(/https?:|ftp:|mailto:/) ) {
			...
			var name = '' + links[i+1];
			if(!name || name.match(/http/) || name == 'undefined' ) name = href;
    ...
    // Insert custom field data -- AO
    rdetailShowCategories();
    rdetailFixPhysicalDescription();
    rdetailShowFirstItemDetails();
...
function rdetailShowExtra(type, args) {
	...
	removeCSSClass($('rdetail_refworks_link'), 'rdetail_extras_selected');
	...
	switch(type) {
		...
		case 'refworks':
		    addCSSClass($('rdetail_refworks_link'), 'rdetail_extras_selected');
		    unHideMe($('rdetail_refworks_div'));
		    rdetailShowRefworks();
		    break;
...
function rdetailBuildBrowseInfo(row, cn, local, orgNode) {
  ...
  // Expand the first 'details' row.
  if ( firstResultRow == null ) {
    firstResultRow = dHref;
  }
}
